/**
 * syncOpenseaOrders.js — API Key olmadan işləyən gücləndirilmiş versiya 🚀
 * OpenSea → Backend NFT order sync
 * Node.js ≥18 (fetch daxili gəlir)
 */

// --- CONFIG ---
const BACKEND_URL = "https://forbidden-nlzj.onrender.com"; 
...
// Retry settings
const RETRY_LIMIT = 5;       // 403 və network xətası üçün retry sayı
const RETRY_DELAY = 5000;    // retry arası gecikmə (ms)
...
async function safeFetch(url, options = {}, attempt = 1) {
  try {
    ...
    if (res.status === 429 || res.status === 403) { // rate-limit və forbidden
...
async function main() {
  ...
  while (true) {
    ...
    offset += PAGE_SIZE;
    await sleep(1000); // server yükünü azaltmaq üçün
  }
